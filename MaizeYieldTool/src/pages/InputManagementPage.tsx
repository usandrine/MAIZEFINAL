...
export const mockInputRecords: InputRecord[] = [
  {
    id: 1,
    date: '2025-04-01',
    field: 'Field A',
    inputType: 'Nitrogen Fertilizer',
    quantity: '50',
    quantityUnit: 'kg',
    unitCost: '1.2',
  ...
  },
  {
    ...
    date: '2025-04-15',
    field: 'Field A',
    inputType: 'Pesticide X',
    quantity: '2',
    quantityUnit: 'liters',
    unitCost: '15',
    supplier: 'CropGuard Inc.',
    applicationMethod: 'Foliar Spray',
    applicationTime: '16:30'
  }
];

function InputManagementPage() {
  const [isAddInputModalOpen, setIsAddInputModalOpen] = useState(false);
  const [selectedInputRecord, setSelectedInputRecord] = useState<InputRecord | null>(null);
  const [isViewDetailsModalOpen, setIsViewDetailsModalOpen] = useState(false);
  const addInputFormRef = useRef<HTMLFormElement>(null);
  ...
  const openViewDetailsModal = (record: InputRecord) => {
  ...
  const validateForm = (formData: FormData) => {
    ...
    if (!formData.get('inputType')) newErrors.inputType = 'Input Type is required';
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  const handleSaveInputRecord = (newRecord: Omit<InputRecord, 'id'>) => {
    console.log('New Input Record:', newRecord);
    mockInputRecords.push({ ...newRecord, id: Date.now() });
    closeAddInputModal();
  };
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (validateForm(formData)) {
      const newRecord: Omit<InputRecord, 'id'> = {
        ...
        applicationTime: formData.get('applicationTime') as string,
      };
            ...
            <tbody>
              {mockInputRecords.map((record) => (
                <tr key={record.id} className="border-b last:border-b-0">
                  ...
                  <td className="py-2 text-green-600 hover:underline cursor-pointer" onClick={() => openViewDetailsModal(record)}>
                    View Details
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" id="addInputModal">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <form ref={addInputFormRef} onSubmit={handleSubmit}>
              {/* You should add actual input fields here if needed */}
            ...
            >
              <svg className="fill-current h-6 w-6" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                <title>Close</title>
                <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l3.029-2.651-3.029-2.651a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-3.031 2.651 3.031 2.651a1.2 1.2 0 0 1 0 1.697z"/>
              </svg>
            </button>
          </div>
        </div>
      ...
      {isViewDetailsModalOpen && selectedInputRecord && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" id="viewDetailsModal">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <div>
              <p><strong>Date:</strong> {selectedInputRecord.date}</p>
              <p><strong>Field:</strong> {selectedInputRecord.field}</p>
              <p><strong>Input Type:</strong> {selectedInputRecord.inputType}</p>
              <p><strong>Quantity:</strong> {selectedInputRecord.quantity} {selectedInputRecord.quantityUnit}</p>
              <p><strong>Unit Cost:</strong> {selectedInputRecord.unitCost} $/unit</p>
              <p><strong>Supplier:</strong> {selectedInputRecord.supplier}</p>
              <p><strong>Application Method:</strong> {selectedInputRecord.applicationMethod}</p>
              <p><strong>Application Time:</strong> {selectedInputRecord.applicationTime}</p>
            </div>
